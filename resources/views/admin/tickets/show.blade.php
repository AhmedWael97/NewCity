...
@section('content')
                            ...
                            <div class="row g-3">
                                @if($ticket->user_id)
                                    <div class="col-md-6">
                                        ...
                                        <strong>
                                            <a href="{{ route('admin.users.show', $ticket->user_id) }}" target="_blank">
                                                {{ $ticket->user->name }}
                                            </a>
                                        </strong>
                                    </div>
                                        ...
                                        <div class="mb-0">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <strong>{{ $ticket->guest_name ?? 'غير متوفر' }}</strong>
                                                </div>
                                                <div class="col-md-6">
                                                    <small class="text-muted d-block">الهاتف</small>
                                                    <strong>
                                                        @if($ticket->guest_phone)
                                                            <a href="tel:{{ $ticket->guest_phone }}">{{ $ticket->guest_phone }}</a>
                                                        @else
                                                            غير متوفر
                                                        @endif
                                                    </strong>
                                                ...
                                                @if($ticket->guest_email)
                                                    <div class="col-md-6 mt-2">
                                                        ...
                                                        <strong><a
                                                                href="mailto:{{ $ticket->guest_email }}">{{ $ticket->guest_email }}</a></strong>
                                                    </div>
                                                @endif
                                                @if($ticket->guest_address)
                                                    <div class="col-md-6 mt-2">
                                                        <small class="text-muted d-block">العنوان</small>
                                                        <strong>{{ $ticket->guest_address }}</strong>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                            ...
                                            @if($isImage)
                                                <div class="attachment-preview">
                                                    <a href="{{ Storage::url($attachment['path']) }}" 
                                                       data-fancybox="ticket-attachments" 
                                            ...
                                            @else
                                                <a href="{{ Storage::url($attachment['path']) }}" target="_blank"
                                                   ...
                                                   style="height: 150px; flex-direction: column;">
                                                    ...
                                                    <span class="text-truncate w-100 px-2">{{ $attachment['name'] }}</span>
                                                    <small class="text-muted mt-1">انقر للتحميل</small>
                                                </a>
                                            @endif
                                        </div>
                                    @endforeach
                    ...
                    <div class="card-body">
                        @if($ticket->replies && $ticket->replies->count() > 0)
                            @foreach($ticket->replies as $reply)
                                <div
                                    class="reply-item mb-3 p-3 {{ $reply->is_internal_note ? 'bg-warning bg-opacity-10' : 'bg-light' }} rounded">
                        ...
                        <form action="{{ route('admin.tickets.reply', $ticket->id) }}" method="POST" class="mt-4">
                            ...
                            <div class="form-check mb-3">
                                <input class="form-check-input" type="checkbox" id="is_internal_note"
                                    name="is_internal_note" value="1">
                        ...
                        <form action="{{ route('admin.tickets.update', $ticket->id) }}" method="POST" class="mb-3">
                            ...
                            <select class="form-select mb-2" id="status" name="status">
                                ...
                                <option value="in_progress" {{ $ticket->status == 'in_progress' ? 'selected' : '' }}>قيد
                                    المعالجة</option>
                                <option value="waiting_user" {{ $ticket->status == 'waiting_user' ? 'selected' : '' }}>في
                                    انتظار المستخدم</option>
                                <option value="resolved" {{ $ticket->status == 'resolved' ? 'selected' : '' }}>تم الحل
                                </option>
    ...
    <script>
        Fancybox.bind('[data-fancybox="ticket-attachments"]', {
            Toolbar: {
                display: {
                    left: ["infobar"],
                    middle: [],
