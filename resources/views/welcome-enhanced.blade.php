@extends('layouts.app')

@section('title', $seoData['title'] ?? 'اكتشف المدن')
@section('description', $seoData['description'] ?? 'اكتشف أفضل المتاجر في مدينتك')
@section('keywords', $seoData['keywords'] ?? 'متاجر, مصر, اكتشف')
@section('og:image', $seoData['og_image'] ?? asset('images/og-discover-cities.jpg'))
@section('canonical', $seoData['canonical'] ?? url()->current())

@section('content')
{{-- City Selection Modal --}}
@if($cityContext['should_show_modal'])
    <x-city-selection-modal :cities="$cities" :show-modal="true" />
@endif

{{-- Hero Section --}}
<section class="relative bg-gradient-to-br from-blue-600 via-purple-600 to-blue-800 text-white">
    <div class="absolute inset-0 bg-black opacity-20"></div>
    
    <div class="relative container mx-auto px-4 py-20">
        <div class="max-w-4xl mx-auto text-center">
            {{-- City Context Display --}}
            @if($cityContext['is_city_selected'])
                <div class="mb-6 inline-flex items-center bg-white bg-opacity-20 rounded-full px-6 py-3 backdrop-blur-sm">
                    <span class="text-lg">📍</span>
                    <span class="mr-3 text-lg font-medium">{{ $cityContext['selected_city_name'] }}</span>
                    <button 
                        onclick="showCityModal()" 
                        class="text-sm text-blue-200 hover:text-white transition-colors duration-200"
                    >
                        تغيير المدينة
                    </button>
                </div>
            @endif
            
            <h1 class="text-5xl md:text-6xl font-bold mb-6">
                @if($cityContext['is_city_selected'])
                    اكتشف أفضل المتاجر في<br>
                    <span class="text-yellow-400">{{ $cityContext['selected_city_name'] }}</span>
                @else
                    اكتشف أفضل المتاجر<br>
                    <span class="text-yellow-400">في مدينتك</span>
                @endif
            </h1>
            
            <p class="text-xl md:text-2xl mb-8 text-blue-100">
                @if($cityContext['is_city_selected'])
                    {{ number_format($stats['total_shops']) }} متجر معتمد في {{ $cityContext['selected_city_name'] }}
                @else
                    {{ number_format($stats['total_shops']) }} متجر معتمد في {{ number_format($stats['total_cities']) }} مدينة مصرية
                @endif
            </p>
            
            {{-- Search Bar --}}
            <div class="max-w-2xl mx-auto">
                <form action="{{ route('search') }}" method="GET" class="relative">
                    <input 
                        type="text" 
                        name="q" 
                        placeholder="ابحث عن متاجر، منتجات، أو خدمات..."
                        class="w-full py-4 px-6 pr-16 text-lg text-gray-800 rounded-2xl shadow-2xl focus:outline-none focus:ring-4 focus:ring-yellow-400"
                        autocomplete="off"
                        id="heroSearch"
                    >
                    <button 
                        type="submit" 
                        class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-yellow-500 hover:bg-yellow-600 text-gray-800 font-bold py-3 px-6 rounded-xl transition-colors duration-200"
                    >
                        بحث
                    </button>
                </form>
                
                {{-- Quick Actions --}}
                <div class="flex flex-wrap justify-center gap-4 mt-6">
                    @unless($cityContext['is_city_selected'])
                        <button 
                            onclick="showCityModal()" 
                            class="bg-white bg-opacity-20 hover:bg-opacity-30 px-6 py-3 rounded-full transition-all duration-200 backdrop-blur-sm"
                        >
                            📍 اختر مدينتك
                        </button>
                    @endunless
                    
                    <a 
                        href="{{ route('categories.index') }}" 
                        class="bg-white bg-opacity-20 hover:bg-opacity-30 px-6 py-3 rounded-full transition-all duration-200 backdrop-blur-sm"
                    >
                        🏪 تصفح الفئات
                    </a>
                    
                    @if($featuredShops->count() > 0)
                        <a 
                            href="#featured-shops" 
                            class="bg-white bg-opacity-20 hover:bg-opacity-30 px-6 py-3 rounded-full transition-all duration-200 backdrop-blur-sm"
                        >
                            ⭐ المتاجر المميزة
                        </a>
                    @endif
                </div>
            </div>
        </div>
    </div>
    
    {{-- Statistics Cards --}}
    <div class="relative -mt-10">
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                <div class="bg-white rounded-xl p-6 text-center shadow-xl">
                    <div class="text-3xl font-bold text-blue-600">{{ number_format($stats['total_shops']) }}</div>
                    <div class="text-gray-600 text-sm">متجر معتمد</div>
                </div>
                
                @unless($cityContext['is_city_selected'])
                    <div class="bg-white rounded-xl p-6 text-center shadow-xl">
                        <div class="text-3xl font-bold text-green-600">{{ number_format($stats['total_cities']) }}</div>
                        <div class="text-gray-600 text-sm">مدينة مصرية</div>
                    </div>
                @else
                    <div class="bg-white rounded-xl p-6 text-center shadow-xl">
                        <div class="text-3xl font-bold text-green-600">{{ number_format($stats['total_products']) }}</div>
                        <div class="text-gray-600 text-sm">منتج متاح</div>
                    </div>
                @endunless
                
                <div class="bg-white rounded-xl p-6 text-center shadow-xl">
                    <div class="text-3xl font-bold text-purple-600">{{ number_format($stats['total_categories']) }}</div>
                    <div class="text-gray-600 text-sm">فئة متنوعة</div>
                </div>
                
                <div class="bg-white rounded-xl p-6 text-center shadow-xl">
                    <div class="text-3xl font-bold text-orange-600">{{ number_format($stats['average_rating'], 1) }}</div>
                    <div class="text-gray-600 text-sm">متوسط التقييم</div>
                </div>
            </div>
        </div>
    </div>
</section>

{{-- Popular Categories Section --}}
@if($popularCategories->count() > 0)
<section class="py-16 bg-gray-50">
    <div class="container mx-auto px-4">
        <div class="text-center mb-12">
            <h2 class="text-4xl font-bold text-gray-800 mb-4">
                @if($cityContext['is_city_selected'])
                    الفئات الشائعة في {{ $cityContext['selected_city_name'] }}
                @else
                    الفئات الشائعة
                @endif
            </h2>
            <p class="text-xl text-gray-600">اكتشف أنواع المتاجر والخدمات المتاحة</p>
        </div>
        
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
            @foreach($popularCategories as $category)
                <a 
                    href="{{ $cityContext['is_city_selected'] ? route('city.category.shops', ['city' => $cityContext['selected_city']->slug, 'category' => $category->slug]) : route('category.shops', $category->slug) }}" 
                    class="group bg-white rounded-xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2"
                >
                    @if($category->icon)
                        <div class="text-4xl mb-4">{{ $category->icon }}</div>
                    @else
                        <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white text-2xl font-bold mx-auto mb-4">
                            {{ mb_substr($category->name, 0, 1) }}
                        </div>
                    @endif
                    
                    <h3 class="font-bold text-gray-800 mb-2 group-hover:text-blue-600 transition-colors">
                        {{ $category->name }}
                    </h3>
                    <p class="text-sm text-gray-500">{{ number_format($category->shops_count) }} متجر</p>
                </a>
            @endforeach
        </div>
    </div>
</section>
@endif

{{-- Featured Shops Section --}}
@if($featuredShops->count() > 0)
<section id="featured-shops" class="py-16 bg-white">
    <div class="container mx-auto px-4">
        <div class="text-center mb-12">
            <h2 class="text-4xl font-bold text-gray-800 mb-4">
                @if($cityContext['is_city_selected'])
                    المتاجر المميزة في {{ $cityContext['selected_city_name'] }}
                @else
                    المتاجر المميزة
                @endif
            </h2>
            <p class="text-xl text-gray-600">اكتشف أفضل المتاجر المعتمدة والموثوقة</p>
        </div>
        
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            @foreach($featuredShops as $shop)
                <div class="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden hover:-translate-y-2">
                    {{-- Shop Image --}}
                    <div class="relative h-48 bg-gradient-to-br from-blue-500 to-purple-600">
                        @php
                            $images = $shop->images_array;
                            $hasImages = is_array($images) && count($images) > 0;
                        @endphp
                        @if($hasImages)
                            <img 
                                src="{{ $images[0] }}" 
                                alt="{{ $shop->name }}"
                                class="w-full h-full object-cover"
                                loading="lazy"
                            >
                        @endif
                        
                        <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
                            <div class="text-white text-center">
                                <h3 class="text-xl font-bold mb-2">{{ $shop->name }}</h3>
                                <div class="flex items-center justify-center text-yellow-400">
                                    @for($i = 1; $i <= 5; $i++)
                                        @if($i <= floor($shop->rating))
                                            ⭐
                                        @endif
                                    @endfor
                                    <span class="text-white mr-2">({{ $shop->review_count }})</span>
                                </div>
                            </div>
                        </div>
                        
                        {{-- Featured Badge --}}
                        <div class="absolute top-4 right-4 bg-yellow-500 text-gray-800 px-3 py-1 rounded-full text-sm font-bold">
                            مميز
                        </div>
                    </div>
                    
                    {{-- Shop Info --}}
                    <div class="p-6">
                        <p class="text-gray-600 mb-4 line-clamp-2">{{ $shop->description }}</p>
                        
                        <div class="flex items-center text-gray-500 text-sm mb-4">
                            <span class="ml-2">📍</span>
                            <span>{{ $shop->address }}</span>
                        </div>
                        
                        <div class="flex items-center justify-between">
                            <span class="text-blue-600 font-medium">{{ $shop->category->name }}</span>
                            <a 
                                href="{{ route('shop.show', $shop->slug) }}" 
                                class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors duration-200"
                            >
                                عرض المتجر
                            </a>
                        </div>
                    </div>
                </div>
            @endforeach
        </div>
        
        @if($cityContext['is_city_selected'])
            <div class="text-center mt-12">
                <a 
                    href="{{ route('city.shops.index', ['city' => $cityContext['selected_city']->slug]) }}" 
                    class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl text-lg font-medium transition-colors duration-200"
                >
                    عرض جميع المتاجر في {{ $cityContext['selected_city_name'] }}
                </a>
            </div>
        @endif
    </div>
</section>
@endif

{{-- City Selection CTA (if no city selected) --}}
@unless($cityContext['is_city_selected'])
<section class="py-16 bg-gradient-to-r from-blue-600 to-purple-600 text-white">
    <div class="container mx-auto px-4 text-center">
        <h2 class="text-4xl font-bold mb-6">اختر مدينتك لتجربة أفضل</h2>
        <p class="text-xl mb-8 max-w-2xl mx-auto">
            احصل على محتوى مخصص لمدينتك واكتشف المتاجر والخدمات القريبة منك
        </p>
        
        <button 
            onclick="showCityModal()" 
            class="bg-yellow-500 hover:bg-yellow-600 text-gray-800 font-bold px-8 py-4 rounded-xl text-lg transition-colors duration-200"
        >
            🏙️ اختر مدينتك الآن
        </button>
    </div>
</section>
@endunless
@endsection

@push('scripts')
<script>
// Search autocomplete functionality
document.getElementById('heroSearch')?.addEventListener('input', function(e) {
    const query = e.target.value.trim();
    
    if (query.length >= 2) {
        fetch(`{{ route('search.suggestions') }}?term=${encodeURIComponent(query)}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json'
            }
        })
        .then(response => response.json())
        .then(data => {
            // Implement autocomplete dropdown here
            console.log('Search suggestions:', data);
        })
        .catch(error => {
            console.error('Search suggestions error:', error);
        });
    }
});

// Analytics tracking
function trackCitySelection(cityName) {
    if (typeof gtag !== 'undefined') {
        gtag('event', 'city_selected', {
            'city_name': cityName,
            'page_location': window.location.href
        });
    }
}

// Track search events
document.querySelector('form[action="{{ route('search') }}"]')?.addEventListener('submit', function(e) {
    const query = document.getElementById('heroSearch').value.trim();
    
    if (typeof gtag !== 'undefined') {
        gtag('event', 'search', {
            'search_term': query,
            'page_location': window.location.href
        });
    }
});
</script>
@endpush